...
if (timer > 128)
{
    ...
    if (roaring_timer < 1)
    {
        if (intensity < 4)
        {
            ...
            if (intensity < 3.75)
                player_suck = scr_approach(player_suck, 3, 0.1625);
        }
    }
    ...
    if (attack_timer == 4)
    {
        rand_angle += (80 + irandom(80));
        rand_dist = 600 + irandom(80);
        ...
        with (scr_fire_bullet(camerax() + fake_x + lengthdir_x(rand_dist, rand_angle + 180), cameray() + fake_y + lengthdir_y(rand_dist, rand_angle + 180), obj_knight_roaring_star, 0, 0, spr_knight_bullet_star))
        {
            wall_destroy = false;
            destroyonhit = false;
            bottomfade = false;
        ...
        if (intensity < 3.5)
            attack_timer = floor(attack_timer_goal + attack_token);
        attack_token = 1 - attack_token;
        attack_timer_goal = scr_approach(attack_timer_goal, 2, 0.1);
    }
}
if (intensity == 4)
{
    ...
    if (roaring_timer < 181)
    {
        ...
        if (roaring_timer > 15 && (roaring_timer % 5) == 0)
        {
            ...
            rand_angle += (40 + irandom(40));
            with (scr_fire_bullet(camerax() + fake_x, cameray() + fake_y + 55, obj_knight_roaring_star, rand_angle, 6.5 + random(2), spr_knight_bullet_star))
            {
                visible = false;
                wall_destroy = false;
                bottomfade = false;
            ...
            with (scr_fire_bullet(camerax() + fake_x, cameray() + fake_y + 55, obj_knight_roaring_star, rand_angle + 80 + random(80), 8.5 + random(2), spr_knight_bullet_star))
            {
                visible = false;
                wall_destroy = false;
                bottomfade = false;
...
if (roaring_timer < 1)
{
    with (obj_knight_roaring_star)
    {
        ...
        {
            if (point_distance(x, y, camerax() + other.fake_x, cameray() + other.fake_y + 55) < (400 + (120 * other.intensity)))
                speed = 2.5 + (4.5 * other.intensity * (1 - (0.000925925925925926 * point_distance(x, y, camerax() + other.fake_x, cameray() + other.fake_y + 55))));
            else
                speed = 2.5 + (1.5 * other.intensity * (1 - (0.000925925925925926 * point_distance(x, y, camerax() + other.fake_x, cameray() + other.fake_y + 55))));
