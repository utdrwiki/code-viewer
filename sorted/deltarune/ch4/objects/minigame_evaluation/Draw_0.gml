...
with (obj_mainchara)
    y = 0;
var challengeresults = stringsetloc("CHALLENGE RESULTS", "obj_round_evaluation_slash_Draw_0_gml_20_0");
var totalscore = stringsetloc("TOTAL SCORE", "obj_round_evaluation_slash_Draw_0_gml_21_0");
var _points = stringsetloc("POINTS", "obj_round_evaluation_slash_Draw_0_gml_22_0");
...
var _norank = stringsetloc("NO RANK", "obj_minigame_evaluation_slash_Draw_0_gml_22_0");
...
if (con == 0.5)
{
    ...
    if (bg_speed_y > -480)
        bg_speed_y -= 480;
    scr_draw_sprite_tiled_area(spr_dw_tv_starbgtile, star_anim, __cx + bg_speed, __cy + bg_speed_y, __cx + bg_speed, __cy + bg_speed_y, __cx + view_wport[0], __cy + room_height, 2, 2, c_white, 1);
}
if (con >= 1)
{
    draw_set_alpha(1);
        ...
        if (gradecon >= 2)
        {
            if (!sndplayed)
            {
                if (button1_p() || button2_p() || button3_p())
                {
                    ...
                    {
                        if (_song_id == 0 && global.flag[1279] < physicalchallengepoints)
                        {
                        ...
                        {
                            scr_rhythmgame_score_save(_song_id, physicalchallengepoints, true);
                        }
                    }
                    else if (_song_id == 0 && global.flag[1195] < physicalchallengepoints)
                    {
                    ...
                    {
                        scr_rhythmgame_score_save(_song_id, physicalchallengepoints);
                    }
                    sndplayed = true;
                }
            }
    ...
    if (replayversion && boardnumber == 1)
    {
        ...
        {
            draw_set_halign(fa_left);
            sp_draw_text(leftx, yy + (mspace * 1), stringsetloc("TALLEST STACK", "obj_round_evaluation_slash_Draw_0_gml_804_0"));
            draw_set_halign(fa_right);
        ...
        {
            draw_set_halign(fa_left);
            sp_draw_text(leftx, yy + (mspace * 2), stringsetloc("FOODS WASTED", "obj_round_evaluation_slash_Draw_0_gml_827_0"));
            draw_set_halign(fa_right);
        ...
        {
            draw_set_halign(fa_left);
            sp_draw_text(leftx, yy + (mspace * 3), stringsetloc("HAPPY CUSTOMERS", "obj_round_evaluation_slash_Draw_0_gml_839_0"));
            draw_set_halign(fa_right);
