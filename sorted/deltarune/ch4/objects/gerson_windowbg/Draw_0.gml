...
{
    lan = scr_marker(camerax() + 106, camerax() + 106, spr_lancer_dt);
    lan.image_alpha = -69;
...
if (drawbell)
{
    if ((timer % 50) == 0)
    {
        for (var i = 0; i < 3; i++)
        {
            ...
            if (i == 1)
                randno = 40;
            var bell = scr_dark_marker(camerax() + 196 + (126 * i) + scr_even(irandom_range(-randno, randno)), cameray() + 172 + scr_even(irandom_range(-20, 20)), spr_bell_small);
            bell.image_alpha = -69;
            scr_doom(bell, 240);
