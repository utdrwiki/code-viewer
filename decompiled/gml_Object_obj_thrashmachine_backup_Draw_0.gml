    ...
    if (bad == 0)
    {
        ...
        if (part[1] == 3)
        {
            ...
            if _flash
            {
                ...
                draw_sprite_ext_glow(spr_thrashweapon_duck, 0, headx, heady, s, s, image_angle, c_white, body_flash, 16777215)
            }
            if _flash
            {
                _af = scr_custom_afterimage_ext(obj_power_up_afterimage, _headsprite, 0, headx, heady, 3, 3)
                _af.use_pivot = 1
                _af.pivotx = ((_pieceVector.x / 3) + 3)
                _af.pivoty = ((_pieceVector.y / 3) - 12)
            }
